...
(function(){
  ...
  function updateTrails(){
    ...
    trails.forEach((t,i)=>{
      const delay = (i+1)*10;
      const h = history.filter(p=> now - p.t > delay*3 && now-p.t < delay*3+40);
      if(h.length){
        const p = h[Math.floor(h.length/2)];
        t.style.left = p.x+'px'; t.style.top = p.y+'px';
        t.style.opacity = (0.45 - i*0.07).toString();
        t.style.width = t.style.height = (4 - i*0.5)+'px';
        t.style.background = i%2===0 ? '#40b4ff' : '#8b5cf6';
      }
    });
  }
  ...
  document.querySelectorAll('button, .nav-link, .feature-card, .option-card, .btn-tool, .back-btn, .team-card, a').forEach(el=>{
...
async function runAnalysis(){
  ...
  if(effectiveLang !== 'python'){
    renderResults(generateDemoResults(code, effectiveLang));
    navigate('results');
    showToast('Local analyzer used for non-Python code.', 'fa-info-circle');
    return;
  }
...
function generateDemoResults(code, lang){
  ...
  if(lang === 'python' || detectLang(code) === 'python'){
  ...
  } else if(lang === 'java' || detectLang(code) === 'java'){
    lines.forEach(l => {
    ...
    lines.forEach(l => {
      const m = l.match(/^(?:public|private|protected)?\s*(?:static\s+)?(?:[\w<>\[\]]+)\s+(\w+)\s*\(/);
      if(m){
        const fn = m[1];
        if(fn !== 'main'){
          const restCode = lines.filter(ll=>ll!==l).join('\n');
          if(!restCode.includes(fn+'(') || restCode.match(new RegExp(`\\b${fn}\\b`, 'g')).length <= 1){
  ...
  } else if(lang === 'cpp' || detectLang(code) === 'cpp'){
    lines.forEach(l => {
